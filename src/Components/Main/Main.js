...
class Main extends React.Component {
    ...
    async makeThisLive() {
        ...
        const currName = Math.random().toString(36).substring(2, 15) + Math.random().toString(36).substring(2, 15);

        const shortUrl = (await axios.post('http://localhost:4000/api/v1/share', {
            "name": currName
        })).data.shortUrl;
